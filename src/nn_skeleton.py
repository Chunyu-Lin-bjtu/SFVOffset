    ...
    with tf.variable_scope(layer_name) as scope:
      ...
      if use_pretrained_param:
        ...
        kernel_init = tf.truncated_normal_initializer(
            stddev=stddev, dtype=tf.float32)
        self.kernel_init_output = kernel_init
        bias_init = tf.constant_initializer(bias_init_val)
      ...
      if self.global_conv_counter==0:
        self.kernel_output = kernel
        self.biases_output = biases
        self.conv_output = conv 
        self.inputs_thing = inputs
      self.global_conv_counter += 1
