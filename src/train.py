    ...
    try:
      for step in xrange(FLAGS.max_steps):
        ...
        if step % FLAGS.summary_step == 0 or step == FLAGS.max_steps-1:
          ...
          op_list = [
              model.lidar_input, model.lidar_mask, model.label, model.train_op,
              model.loss, model.pred_cls, model.output_prob, summary_op
          ]
          lidar_per_batch, lidar_mask_per_batch, label_per_batch, \
            _, loss_value, pred_cls, output_prob_per_batch,summary_str = sess.run(op_list,options=run_options) 
          '''
          print('--------------------------------------------------------------')
          print('lidar_per_batch.shape: ', lidar_per_batch.shape)
          print('lidar_mask_per_batch.shape: ', lidar_mask_per_batch.shape)
          print('label_per_batch.shape: ', label_per_batch.shape)
          print('loss_value.shape: ', loss_value.shape)
          print('pred_cls.shape:', pred_cls.shape)
          print('output_prob_per_batch.shape:', output_prob_per_batch.shape)
          # print('output_prob_per_batch:\n', output_prob_per_batch)
          new_label_per_batch = tf.reshape(label_per_batch,(-1,))
          new_output_prob_per_batch = tf.reshape(output_prob_per_batch,(-1,mc.NUM_CLASS))
          print('new_label_per_batch.shape:',new_label_per_batch.shape)
          print('new_output_prob_per_batch.shape:',new_output_prob_per_batch.shape)
          print('new_label_per_batch.shape:',new_label_per_batch.shape)
          print('new_label_per_batch.max:',sess.run(tf.reduce_max(new_label_per_batch)))
          print('new_output_prob_per_batch.shape:',new_output_prob_per_batch.shape)
          print('new_output_prob_per_batch.max:',sess.run(tf.reduce_max(new_output_prob_per_batch)))
          # print('num_class:',mc.NUM_CLASS)
          '''
          # my_loss_val = tf.identity(tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits()))
