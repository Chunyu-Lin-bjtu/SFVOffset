...
def eval_once(
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        iou_write = open('train_iou_0_9.txt','w')
        for i in xrange(int(num_images/mc.BATCH_SIZE)):
            ...
            write_iou_string = str(file_idx_name) + ' ' + str(iou[1]) + ' ' + \
                str(iou[2]) + ' ' + str(iou[3]) + '\n'
            print("write_iou_str:",write_iou_string)
            iou_write.write(write_iou_string)
            ...
            points_info = np.zeros([pred_cls_.shape[0],pred_cls_.shape[1],6])
            points_info[:,:,0:5] = lidar_per_batch_[0,:,:,0:5]
            points_info[:,:,5] = pred_cls_
