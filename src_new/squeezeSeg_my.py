...
class SqueezeSeg(ModelSkeleton):
  ...
  def _add_forward_graph(self):
    ...
    # -----------------------zy add it ------------------
    fire10_fuse = tf.concat([fire10_fuse, fire_ASPP],3,name = 'concat64x64')
  ...
  def atrous_spatial_pyramid_pooling(self,net, scope, depth=128):
    with tf.variable_scope(scope):
      ...
      # original # at_pool3x3_1 =  self._conv_layer("conv_3x3_1",net, depth,size=3,stride=[1,1],padding='SAME',dilations=[1,6,6,1],freeze=False,relu=True,BN=False)
      at_pool3x3_1 =  self._conv_layer("conv_3x3_1",net, depth,size=3,stride=[1,1],padding='SAME',dilations=[1,2,6,1],freeze=False,relu=True,BN=False)
      #  tf.summary.image('at_pool3x3_1',tf.slice(at_pool3x3_1,[0,0,0,0],[shape[0],shape[1],shape[2],3]),max_outputs=1)
      at_pool3x3_2 = self._conv_layer('conv_3x3_2',net, depth, size=3,stride=[1,1],padding='SAME',dilations=[1,3,9,1],freeze=False,relu=True,BN=False)
      #    tf.summary.image('at_pool3x3_2',tf.slice(at_pool3x3_2,[0,0,0,0],[shape[0],shape[1],shape[2],3]),max_outputs=1)
      at_pool3x3_3 = self._conv_layer('conv_3x3_3',net, depth, size=3,stride=[1,1],padding='SAME',dilations=[1,4,12,1],freeze=False,relu=True,BN=False)
